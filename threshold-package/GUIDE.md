...
Stand-in** menu on the row. That is how the demo ships — every photograph you see
in it is drawn by the component, which is why the download has no photo folder.
...
the layout, the interactions, the drawn photographs — reads from that data.
...
**The photographs in the demo are drawn.** Every interior, exterior and portrait
you see is generated by the component from a short specification. Upload your own
and they are replaced. This is why the site loads with no image requests at all,
and why the download has no photo folder to sort through.
