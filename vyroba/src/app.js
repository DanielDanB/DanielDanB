...
function slotEnd(sl) {                              // poslední den a hodina konce
  var H = shiftH(), e = slotA(sl) + (+sl.hours || 0), i = Math.floor((e - 1) / H);
...
function viewCapacity(root) {
  ...
  var slots = [];
  S.orders.forEach(function (o) {
    (o.slots || []).forEach(function (s) { if (laneOf(s.center)) slots.push({ o: o, s: s, A: slotA(s) }); });
  });
  var inRange = slots.filter(function (x) { return x.A < rangeA + PB_DAYS * H && x.A + (+x.s.hours || 0) > rangeA; });
  ...
  LANES.forEach(function (l) {
    ...
    mine.forEach(function (x) {
      ...
      ends[r] = x.A + (+x.s.hours || 0); x.row = r;
    });
    ...
    mine.forEach(function (x) {
      var a = x.A - rangeA, b = a + (+x.s.hours || 0);
      for (var d2 = Math.max(0, Math.floor(a / H)); d2 < PB_DAYS && d2 * H < b; d2++) {
        load[d2] += Math.max(0, Math.min(b, (d2 + 1) * H) - Math.max(a, d2 * H));
      }
    });
    var total = load.reduce(function (a, b) { return a + b; }, 0);
    var over = load.filter(function (v) { return v > H + 1e-9; }).length;
    ...
    var lbl = el('div', 'pb-lbl', '<b>' + l.c + '</b><span>' + mine.length + ' buněk · ' + fmtH(total) + '</span>' +
      (over ? '<span style="color:var(--bad);font-weight:600">' + over + ' ' + (over === 1 ? 'den přetížen' : over < 5 ? 'dny přetížené' : 'dní přetížených') + '</span>' : '<span>kapacita ' + fmtH(H) + ' / den</span>'));
    ...
    for (var d3 = 0; d3 < PB_DAYS; d3++) {
      ...
      if (load[d3] > 0) {
        var lc = el('div', 'pb-load' + (load[d3] > H + 1e-9 ? ' over' : load[d3] >= H - 1e-9 ? ' full' : ''),
          dayW >= 34 ? (Math.round(load[d3] * 10) / 10).toLocaleString('cs') + (dayW >= 52 ? ' / ' + H + ' h' : '') : '');
        lc.style.cssText = 'left:' + d3 * dayW + 'px;width:' + dayW + 'px';
        lc.title = fmtDay(ds3) + ' — ' + l.c + ' ' + fmtH(load[d3]) + ' z ' + fmtH(H);
        lc.appendChild(el('i')).style.width = Math.min(100, load[d3] / H * 100) + '%';
        trk.appendChild(lc);
      }
    }
...
function slotEl(x, l, rangeA, ppH) {
  ...
  b.addEventListener('pointerdown', function (e) {
    ...
    var x0 = e.clientX, A0 = x.A, h0 = +sl.hours || 1, dh = 0;
    ...
    function mv(ev) {
      ...
      if (resize) {
        var nh = Math.max(1, h0 + dh);
        b.style.width = Math.max(8, nh * ppH - 2) + 'px';
    ...
    function up() {
      ...
      if (resize) sl.hours = Math.max(1, h0 + dh); else setSlotA(sl, A0 + dh);
...
function openSlot(o, sl) {
  ...
  function preview() {
    var t = { start: di.value || sl.start, hour: +hs.value, hours: Math.max(0.5, +li.value || 0.5) };
  ...
  sp.onclick = function () {
    ...
    sl.hours = h1; setSlotA(nw, slotA(sl) + h1);
    o.slots.push(nw);
