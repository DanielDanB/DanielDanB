...
function aresFailed(err, ico) {
  modal({
    title: 'ARES neodpověděl',
    text: err.message + (S.server.ares ? '' : ' Prohlížeče běžně nedovolí volat ARES přímo z cizí stránky. Spolehlivé řešení je mezikrok na vašem serveru — hotový je ve složce server/ v repozitáři.'),
    ok: S.server.ares ? 'Zavřít' : 'Nastavit mezikrok',
    cancel: 'Zadat ručně'
  }).then(function (v) {
    if (v === true && !S.server.ares) { S.view = 'dict'; render(); setTimeout(function () { var f = $('#aresField'); if (f) { f.focus(); f.scrollIntoView({ block: 'center' }); } }, 60); }
    else if (v === null) editCustomer({ ico: String(ico || '').replace(/\D/g, '') }, true);
  });
}
...
function customerPanel() {
  ...
  src.innerHTML = S.server.ares ? 'přes váš server' : 'přímo z prohlížeče';
  ...
  aw.appendChild(ai);
  aw.appendChild(el('p', 'note', 'Bez něj se ARES volá přímo z prohlížeče a ten volání zpravidla odmítne. ' +
    'Server musí na <code>GET &lt;adresa&gt;/{IČO}</code> vrátit odpověď ARESu. ' +
    'Hotový k nasazení je ve složce <b>server/</b> v repozitáři (<code>node ares-proxy.js</code>).'));
