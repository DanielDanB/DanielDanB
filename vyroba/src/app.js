...
function aresFailed(err, ico) {
  ...
  modal({
    ...
    text: err.message + (offerHop ? ' Spolehlivě to vyřeší mezikrok na vašem serveru — hotový je ve složce server/ v repozitáři.' : ''),
    ok: offerHop ? 'Nastavit mezikrok' : 'Zavřít',
    cancel: 'Zadat ručně'
  }).then(function (v) {
    if (v === true && offerHop) {
      S.view = 'dict'; render();
...
function aresSources(ico) {
  ...
  src.push({ name: 'ARES přímo', url: 'https://' + ARES_PATH + ico });
  src.push({ name: 'veřejný mezikrok', url: 'https://r.jina.ai/http://' + ARES_PATH + ico, loose: true });
  return src;
}
...
function aresLookup(icoRaw) {
  ...
  function attempt(i) {
    ...
    var s = src[i];
    return fetch(s.url, { headers: { Accept: 'application/json' } })
      .then(function (r) {
        if (r.status === 404) {
...
function customerPanel() {
  ...
  src.textContent = S.server.ares ? 'pořadí: váš server → ARES → veřejný mezikrok'
    : 'pořadí: ARES přímo → veřejný mezikrok';
  ...
  aw.appendChild(ai);
  aw.appendChild(el('p', 'note', 'Nepovinné. Bez něj se zkouší ARES napřímo a pak veřejný mezikrok r.jina.ai. ' +
    'Vlastní server je spolehlivější a IČO neputuje přes cizí službu — musí na ' +
    '<code>GET &lt;adresa&gt;/{IČO}</code> vrátit odpověď ARESu. Hotový je ve složce <b>server/</b> ' +
    'v repozitáři (<code>node ares-proxy.js</code>).'));
